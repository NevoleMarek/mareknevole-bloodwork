...
export type ReadingWithMeasurements = Awaited<
  ReturnType<typeof getReadingsWithMeasurements>
>[number];

export interface SupplementUpdateInput {
  readonly id: string;
  readonly name: string;
  readonly dose: string;
  readonly frequency: string;
  readonly startedAt: string;
  readonly changelogDate: string;
}
...
export interface SupplementDeleteInput {
  readonly id: string;
  readonly changelogDate: string;
}
...
export interface RepositoryContract {
  readonly getVocabulary: () => Effect.Effect<
    ...
    PersistenceError
  >;
  readonly getLabOverview: () => Effect.Effect<
    Awaited<ReturnType<typeof getLabOverview>>,
    PersistenceError
  >;
  readonly getBiomarkerTrend: (
    key: string,
  ) => Effect.Effect<
    Awaited<ReturnType<typeof getBiomarkerTrend>>,
    PersistenceError
  >;
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const getReadingsEffect = Effect.fn(
    ...
    const deleteReadingEffect = Effect.fn("Repository.deleteReading")(
      function* (id: string) {
        const result = yield* d1(
          "Repository.deleteReading",
          (db) =>
            db.prepare("DELETE FROM readings WHERE id = ?").bind(id).run(),
          database,
        );
        if (result.meta.changes === 0) {
          return yield* Effect.fail(
            new NotFoundError({ resource: "reading", id }),
          );
        }
      },
    );
      ...
      yield* d1Mutation(
        ...
        (db) => {
          ...
          statements.push(
            db
              .prepare(
                "INSERT INTO readings (id, date, source) VALUES (?, ?, ?)",
              )
